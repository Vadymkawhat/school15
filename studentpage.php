<?php
    ...
    require_once '/php/class_student.php';
    $sSurname = filter_var(trim($_POST['studentsurname']), FILTER_SANITIZE_STRING);
    //sql запит
...
<?php
  ...
  <header>
    ...
    <?php endif; ?>
        <section>
        <div class="container-xxl">
                    <form action="studentpage.php" class="d-flex w-75 mx-auto" method="post">
                      <input class="form-control me-3" type="search" placeholder="Введіть прізвище учня..." aria-label="Search" name="studentsurname">
  ...
  ?>
      ...
      foreach($posts as $info){
        $mystudent = new Student($info['name'], $info['surname'], $info['patronymic'], $info['birthday'], $info['file_name']);
          if(mb_strtolower($mystudent->perSurname, "UTF-8") == mb_strtolower($sSurname, "UTF-8")){
  ?>
