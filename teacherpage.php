<?php
    ...
    require '/php/class_teacher.php';
    // $tSurname = 'Кравченко';
    $tSurname = filter_var(trim($_POST['teachersurname']), FILTER_SANITIZE_STRING);
    //sql запит
    $query = "SELECT * FROM `school2`.`teacher`";
...
<?php
  ...
  <header>
    ...
    <?php endif; ?>
        <section>
        <div class="container-xxl">
                    <form action ="teacherpage.php" class="d-flex w-75 mx-auto" method="post">
                      <input class="form-control me-3" type="search" placeholder="Введіть прізвище вчителя..." aria-label="Search" name="teachersurname" id="teachersurname">
  ...
  <?php
      ...
      foreach($posts as $info){
        $myteacher = new Teacher($info['name'], $info['surname'], $info['patronymic'], $info['birthday'], $info['file_name'], $info['education'], $info['experience']);
          if(mb_strtolower($myteacher->perSurname, "UTF-8") == mb_strtolower($tSurname, "UTF-8")){
  ?>
     <div class="card mt-3 w-75 rounded mx-auto d-block">
                <div class="row g-0">
                   <div class="col-md-4">
